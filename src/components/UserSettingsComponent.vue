...
export default defineComponent({
  ...
  methods: {
    async handleSubmit() {
      axios.post(`${import.meta.env.VITE_PROTOCOL}://${import.meta.env.VITE_HOST}/api/dlt/store-github-link`, {
        ...
        axios.post(`${import.meta.env.VITE_PROTOCOL}://${import.meta.env.VITE_HOST}/api/spider/set-tokens`, {
  ...
  async mounted() {
    this.request_data.user.gh_username = (await axios.get(`${import.meta.env.VITE_PROTOCOL}://${import.meta.env.VITE_HOST}/api/dlt/get-github-link`)).data.slice(19).slice(0, -4);
    const { data } = await axios.get(`${import.meta.env.VITE_PROTOCOL}://${import.meta.env.VITE_HOST}/api/spider/get-tokens`);
    this.request_data.user.gh_token = data.github_token;
    this.request_data.user.libraries_token = data.libraries_token;
    this.request_data.user.dlt_gpg = (await axios.get(`${import.meta.env.VITE_PROTOCOL}://${import.meta.env.VITE_HOST}/api/dlt/get-gpg-key`)).data;
  },
});
</script>
