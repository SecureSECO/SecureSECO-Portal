<template>
  <div class="row">
    <div class="flex xs6">
      <div class="row">
        <div class="flex xs4 propName">name:</div>
        <div class="flex xs8 propValue">{{ miner.name }}</div>
      </div>
      <div class="row">
        <div class="flex xs4 propName">id:</div>
        <div class="flex xs8 propValue">{{ miner.id }}</div>
      </div>
      <div class="row">
        <div class="flex xs4 propName">config:</div>
        <div class="flex xs8 json">{{ miner.config }}</div>
      </div>
      <div class="row">
        <div class="flex xs4 propName">state:</div>
        <div class="flex xs8 json">{{ miner.state }}</div>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import { defineComponent } from 'vue';
import { defaultMiner } from '@/api';

export default defineComponent({
  name: 'miner-details-component',
  props: {
    id: {
      type: String,
      required: true,
    },
  },
  data() {
    return {
      miner: defaultMiner,
    };
  },
  // X: computed: {
  // X:   // TODO: Add links for other Platforms and generalise
  // X:   githubLink() {
  // X:     if (this.package.platform !== 'github') {
  // X:       return undefined;
  // X:     }
  // X:     return `https://github.com/${this.package.owner}/${this.package.name}`;
  // X:   },
  // X: },
  // X: watch: {
  // X:   async version() {
  // X:     await this.updateScore();
  // X:   },
  // X: },
  async mounted() {
    this.miner = await this.$searchApi.getMiner(this.id);
  },
  // X: methods: {
  // X:   selectVersion(version: string) {
  // X:     router.push({
  // X:       name: 'Package with Version',
  // X:       params: {
  // X:         name: this.name,
  // X:         version,
  // X:       },
  // X:     });
  // X:   },
  // X:   async updateScore() {
  // X:     this.score = await this.$dltApi.getTrustScore(this.name, this.version);
  // X:   },
  // X: },
});
</script>

<style lang="scss">
.propName {
  text-align: right;
}

.json {
  display: block;
  unicode-bidi: embed;
  font-family: monospace;
  white-space: pre;
}

.va-radio {
  margin: 0 !important;

  .va-radio__icon {
    display: none;
  }

  .va-radio__text {
    margin-left: 0;
  }
}

.va-button {
  margin: 2px;
}
</style>

<!-- This program has been developed by students from the bachelor Computer Science at Utrecht University within the Software Project course.
© Copyright Utrecht University (Department of Information and Computing Sciences) -->
